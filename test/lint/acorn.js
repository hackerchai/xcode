...
(function(exports) {
  ...
  // in turn calls the function that parses `[]` subscripts 钬?that
  ...
  // Verify that a node is an lval 钬?something that can be assigned
  // to.
  ...
  // Parse an atomic expression 钬?either a single token that is an
  ...
  // to be a `[]` or dot subscript expression, but not a call 钬?at
  ...
  function parseObj() {
    ...
    while (!eat(_braceR)) {
      ...
      // getters and setters are not allowed to clash 钬?either with
      // each other or with an init property 钬?and in strict mode,
