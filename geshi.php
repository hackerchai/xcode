 ...
 * be generous here...
 * but keep in mind, that when reaching this limit we
...
class GeSHi
{
     ...
     * NOTE: Used with Escape Char Sequences to fix UTF-8 handling (cf.
     * SF#2037598)
     *
     * @var string
     */
     ...
     * @param
     *            string The path to the language file directory. <b>This
     ...
     *
     * @return string false error message if there has been an error, else false
     * @since 1.0.0
     */
    function error()
    {
     ...
     * @param
     *            string|boolean If a string, this is the style of the line
     ...
     * @param
     *            boolean If set, is the flag for whether to merge the "fancy"
     ...
     * Defaults to true to reduce overhead.
     ...
     * @param
     *            boolean Whether to merge the new styles with the old or just
     *            to overwrite them
     * @since 1.0.0
     */
     ...
     * $key is a number that references the
     ...
     * An optional lookup table can be
     ...
     * @param
     *            bool Wether to automatically recompile the optimized regexp list or not.
     ...
     * @param
     *            string The URL to set for the group. If {FNAME} is in
     ...
     *
     * @param
     *            int A constant that specifies what state the style is being
     ...
     *
     * @param
     *            mixed An array of line numbers to highlight, or just a line
     *            number on its own.
     * @param
     *            string A string specifying the style to use for this line.
     ...
     * This is automatically called in parse_code() when appropriate.
...
( ! $this -> language_data[ 'CASE_SENSITIVE' ][ GESHI_COMMENTS ] && ( ( $match_i = strpos( $part , $comment_mark , $i ) ) !== false ) ) )
     ...
     * Set the _hmr_* vars before calling this
     *
     * @note this is a callback, don't use it directly
     *
     * @param
        ...
        foreach( array_keys( $this -> language_data[ 'KEYWORDS' ] ) as $k ){
            if( ! isset( $this -> lexic_permissions[ 'KEYWORDS' ][ $k ] ) || $this -> lexic_permissions[ 'KEYWORDS' ][ $k ] ){
                ...
                for( $set = 0 , $set_length = count( $this -> language_data[ 'CACHED_KEYWORD_LISTS' ][ $k ] ) ; $set < $set_length ; ++ $set ){
                    ...
                    $stuff_to_parse = preg_replace_callback( "/$disallowed_before_local({$keywordset})(?!\<DOT\>(?:htm|php|aspx?))$disallowed_after_local/$modifiers" , array( 
                            $this , 
    ...
    function header()
    {
        ...
        if( $this -> line_numbers != GESHI_NO_LINE_NUMBERS ){
            ...
            } elseif( $this -> header_type == GESHI_HEADER_DIV || $this -> header_type == GESHI_HEADER_PRE_VALID ){
     ...
     *
     * @param
     *            int The line number information is requested for
     * @access private
     * @since 1.0.7.21
     */
     ...
     * @param $list array
     *            of (unquoted) strings
     * @param $regexp_delimiter your
     *            regular expression delimiter, @see preg_quote()
                    ...
                    if( $char > 0 ){
                        ...
                        if( $char == strlen( $prev_keys[ $level ] ) ){
                            ...
                            if( in_array( $new_key_part1[ 0 ] , $regex_chars ) || in_array( $new_key_part2[ 0 ] , $regex_chars ) ){
                                ...
                                $pointer[ $new_key_part1 ] = array( 
                                        $new_key_part2 => $pointer[ $prev_keys[ $level ] ] 
                                );
     ...
     * @param $recursed bool
     *            to know wether we recursed or not
     * @return string
     * @author Milian Wolff <mail@milianw.de>
     * @since 1.0.8
     * @access private
...
if( ! function_exists( 'geshi_highlight' ) ){
     ...
     * Behaves just like highlight_string
     *
     * @param
     *            string The code to highlight
     * @param
     *            string The language to highlight the code in
     * @param
     *            string The path to the language files. You can leave this blank if you need
     *            as from version 1.0.7 the path should be automatically detected
     * @param
     *            boolean Whether to return the result or to echo
